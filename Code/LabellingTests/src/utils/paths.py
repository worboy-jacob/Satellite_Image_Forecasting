...
def get_logs_dir() -> Path:
    ...
    return BASE_DIR / "Code" / "LabellingTests" / "logs"
...
def get_config_dir() -> Path:
    ...
    return BASE_DIR / "Code" / "LabellingTests" / "config"
...
def get_grid_path() -> Path:
    """Return the grid of country directory."""
    return get_results_dir() / "SatelliteImageData" / "grids" / "Senegal_grid_5km.gpkg"


def get_wealthindex_dir() -> Path:
    """Return the wealth index directory"""
    return get_results_dir() / "GPSResults" / "WealthGPS"


def get_wealthindex_paths() -> Path:
    """Return the specific files of all the wealth indices for each year."""
    return [
...
def get_output_dir() -> Path:
    """Return the location for storing the results."""
    return get_results_dir() / "LabelTestResults"
