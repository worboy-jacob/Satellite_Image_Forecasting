...
def repair_sentinel_band_failure(
    cell_id: int,
    band: str,
    year: int,
    country_name: str,
    ...
    """
    Repair a failed band download for Sentinel data.

    Args:
        cell_id: ID of the cell
        band: Band that failed
    ...
    """
    logger.info(
        f"Attempting to repair band {band} for cell {cell_id}, {country_name} {year}"
    )
    ...
    try:
        ...
        # Check if we have a merged collection already (from previous processing)
        processed_data_file = cell_dir / "processed_data.npz"
        if processed_data_file.exists():
            # We have processed data, but need to repair a specific band
            # First, let's check if we have the original data directory
            ...
            # Build a collection for each month
            all_collections = []
            images_per_month = 5
            ...
            for start_date, end_date in date_ranges:
                try:
                    ...
                    if count > 0:
                        ...
                        # Take up to images_per_month
                        month_selection = sorted_collection.limit(images_per_month)
                        all_collections.append(month_selection)
                except Exception as e:
                    logger.warning(
                        f"Error processing month {start_date} to {end_date}: {e}"
                    )
                    # Continue to next month

            if not all_collections:
                logger.error(f"No collections found for cell {cell_id}")
                repair_logger.log_repair_attempt(
                    cell_id,
                    "band_download_failed",
                    "Failed to get collections for band repair",
                    "failed",
                    {"band": band, "year": year},
                )
                return False
            ...
            merged_collection = all_collections[0]
            ...
            # Download the band with retries
            max_retries = 8
            retry_delay = 2
            tmp_path = None
            ...
            for attempt in range(max_retries):
                try:
                    ...
                    with rasterio.open(tmp_path) as src:
                        ...
                        if band_array.shape[0] < 10 or band_array.shape[1] < 10:
                            ...
                            repair_logger.log_repair_attempt(
                                ...
                                f"Unexpected resolution: {band_array.shape}",
                                "failed",
                    ...
                    # First load existing band arrays
                    existing_bands = {}

                    # Check for existing band files
                    for existing_band in BAND_RESOLUTION.keys():
                        existing_band_file = original_dir / f"{existing_band}.tif"
                        if existing_band_file.exists() and existing_band != band:
                            with rasterio.open(existing_band_file) as src:
                                existing_bands[existing_band] = src.read(1)

                    # Add our newly downloaded band
                    existing_bands[band] = band_array

                    # Process and save
                    npz_path = process_and_save_bands(
                        band_arrays=existing_bands,
                    ...
                    if npz_path is None:
                        logger.error(
                            f"Failed to process and save bands for cell {cell_id}"
                        )
                        repair_logger.log_repair_attempt(
                            cell_id,
                    ...
                    # Log success
                    logger.info(f"Successfully repaired band {band} for cell {cell_id}")
                    repair_logger.log_repair_attempt(
                        cell_id,
                        "band_download_failed",
                        f"Original band download failed",
                        "success",
                        {"band": band, "shape": band_array.shape},
                    )
            ...
            # No processed data exists yet, need to do full cell processing
            logger.info(
                f"No processed data found for cell {cell_id}, attempting full cell repair"
            )
            return repair_sentinel_cell_failure(
                cell_id=cell_id,
                year=year,
                country_name=country_name,
                grid_gdf=grid_gdf,
        ...
        logger.error(f"Error repairing band {band} for cell {cell_id}: {e}")
        repair_logger.log_repair_attempt(
            cell_id, "band_repair_error", str(e), "failed", {"band": band}
        )
        return False
...
def repair_viirs_band_failure(
    ...
    """
    Repair a failed band download for VIIRS data.

    Args:
        cell_id: ID of the cell
        band: Band that failed (usually 'avg_rad')
        year: Year of the data
        country_name: Name of the country
        grid_gdf: Grid GeoDataFrame
        target_crs: Target CRS
    ...
    """
    logger.info(
        f"Attempting to repair VIIRS band {band} for cell {cell_id}, {country_name} {year}"
    )
    ...
    try:
        ...
        # Check if we have a processed data file already
        processed_data_file = cell_dir / "processed_data.npz"
        if processed_data_file.exists():
            # We have processed data, but need to repair a specific band
            # First, let's check if we have the original data directory
            ...
            for attempt in range(max_retries):
                try:
                    ...
                    # Add gradient if this is the avg_rad band
                    if band == "avg_rad":
                        try:
                            gradient = calculate_nightlight_gradient(band_array)
                    ...
                    if npz_path is None:
                        logger.error(
                            f"Failed to process and save VIIRS bands for cell {cell_id}"
                        )
                        repair_logger.log_repair_attempt(
                            cell_id,
                            "band_processing_error",
                    ...
                    # Log success
                    logger.info(
                        f"Successfully repaired VIIRS band {band} for cell {cell_id}"
                    )
                    repair_logger.log_repair_attempt(
                        cell_id,
                        "band_download_failed",
                    ...
                    logger.error(f"Error downloading VIIRS band {band}: {e}")
                    # Clean up temporary file if it exists
                    if tmp_path and os.path.exists(tmp_path):
                        try:
                            os.unlink(tmp_path)
                    ...
                    if attempt < max_retries - 1:
                        ...
                        logger.error(
                            f"Failed to download VIIRS band {band} after {max_retries} attempts"
                        )
                        repair_logger.log_repair_attempt(
                            cell_id,
                            "band_download_failed",
            ...
            # No processed data exists yet, need to do full cell processing
            logger.info(
                f"No processed data found for VIIRS cell {cell_id}, attempting full cell repair"
            )
            return repair_viirs_cell_failure(
                cell_id=cell_id,
                year=year,
                country_name=country_name,
                grid_gdf=grid_gdf,
        ...
        logger.error(f"Error repairing VIIRS band {band} for cell {cell_id}: {e}")
        repair_logger.log_repair_attempt(
            cell_id, "band_repair_error", str(e), "failed", {"band": band}
        )
        return False
...
def repair_failures_by_type(
    ...
    """
    Repair failures of a specific type.
    ...
    Args:
        ...
        early_year: Whether this is an early year (pre-2017) for Sentinel
    ...
    for cell_id, cell_failures in failures_by_cell.items():
        ...
        # Attempt to repair based on failure type
        success = False

        if data_type == "sentinel":
            if (
                failure_type == "band_download_failed"
                or failure_type == "band_resolution_error"
            ):
                # Find which band failed
                band = None
                ...
                if band:
                    success = repair_sentinel_band_failure(
                        cell_id=cell_id,
                        band=band,
                        year=year,
                        country_name=country_name,
                    ...
                    # If we can't determine the band, try repairing the whole cell
                ...
                # These require full cell repair
                success = repair_sentinel_cell_failure(
                    cell_id=cell_id,
                    year=year,
                    country_name=country_name,
                ...
                # Unknown failure type, try full cell repair
                logger.warning(
                    f"Unknown failure type: {failure_type}, attempting full cell repair"
                )
                success = repair_sentinel_cell_failure(
                    cell_id=cell_id,
                    year=year,
            ...
            if (
                ...
                if band:
                    success = repair_viirs_band_failure(
                        cell_id=cell_id,
                        band=band,
                        year=year,
                        country_name=country_name,
        ...
        if success:
            ...
            results["cells"][str(cell_id)] = {"status": "repaired"}
        else:
            results["failed_repairs"] += 1
            results["cells"][str(cell_id)] = {"status": "failed"}
